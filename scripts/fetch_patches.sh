...
# Reject upstream error pages or incomplete payloads before replacing the dataset.
python3 - "$TMP_OUT" <<'PY'
import json
import sys
from pathlib import Path
...
groups = data.get("Product") if isinstance(data, dict) else None
if not isinstance(groups, list) or not groups:
    raise SystemExit("Invalid dataset: expected nonempty Product list")
if not all(isinstance(group, dict) and isinstance(group.get("patches"), list) for group in groups):
    raise SystemExit("Invalid dataset: expected patches lists")
if not any(group["patches"] for group in groups):
    raise SystemExit("Invalid dataset: no patches; preserving previous dataset")
PY
