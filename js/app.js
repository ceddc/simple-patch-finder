...
function getSingleProductValue() {
...
function isExactProductListingRoute() {
  return !patchRoute.pid && !!getSingleProductValue() && state.filters.products.size === 1 && !hasAdditionalListingFilters();
}
...
function buildSeoMetadata() {
  ...
  const base = "https://simplepatchfinder.ceddc.dev/";
  const product = getSingleProductValue();
  ...
  if (patch) {
    ...
    const title = patchName && patchId
      ? `${patchName} (${patchId}) | Simple Patch Finder`
      : patchName
        ? `${patchName} | Simple Patch Finder`
    ...
    return {
      title,
      description,
      robots: patchName && sanitizeHttpUrl(patch.patchPageUrl) ? "index,follow" : "noindex,follow",
...
function ensureGrid() {
    ...
    columns: [
      {
        ...
        formatter: (cell) => {
          ...
          return `<div class="patch-name"><div><a class="patch-link" href="${escapeAttr(buildPatchPermalinkUrl(d))}">${name}</a></div><div class="patch-meta"><span class="mono">${qfe}</span><span>${rel}</span></div></div>`;
        },
      },
      {
        title: "Products",
  ...
  els.grid.addEventListener("click", (event) => {
    const link = event.target.closest("a.patch-link");
    if (!link || event.button !== 0 || event.ctrlKey || event.metaKey || event.shiftKey || event.altKey) return;
    event.preventDefault();
    const patch = findPatchByRoute(readPatchRouteFromParams(new URL(link.href).searchParams));
    if (patch) openPatch(patch);
  });
